...
def crear_master(run_id):
    user_data = leer_script("user_data_master.sh")
    nombre = f"Kafka-Master-{run_id}"
        ...
        TagSpecifications=[
            {
                ...
                "Tags": tags_base(nombre, "KafkaMaster", run_id, indice="master"),
            }
        ],
    )
...
def crear_worker(indice, broker_ip, run_id):
    ...
    instancias = ec2_resource.create_instances(
        ...
        TagSpecifications=[
            {
                "ResourceType": "instance",
                "Tags": tags_base(nombre, "KafkaWorker", run_id, indice=indice),
            }
        ],
    )
...
def crear_workers(cantidad, broker_ip, run_id):
    workers = []
    for indice in range(1, cantidad + 1):
        workers.append(crear_worker(indice, broker_ip, run_id))
    return workers
...
def levantar_cluster(cantidad_workers):
    ...
    print(f"Se crearan {cantidad_workers + 1} instancia(s) EC2 nuevas: 1 master Kafka y {cantidad_workers} worker(s).")
    print(f"Identificador del cluster: {run_id}")

    master = crear_master(run_id)
    workers = crear_workers(cantidad_workers, master.private_ip_address, run_id)

    print("\nCluster Kafka solicitado correctamente.")
    ...
    print("  /home/ec2-user/kafka_demo.sh")
    print("\nConceptos demostrados: Producer, Consumer, Topic, Partition, Offset y Consumer Group.")

    ids = [master.id] + [worker.id for worker in workers]
    print("\nPara borrar estas instancias:")
    print("  python3 src/levantar_kafka.py --delete")
...
def verificar_cluster():
    ...
    if not instancias:
        print("No hay instancias Kafka creadas por este script.")
        return

    print("Instancias Kafka:")
...
def destruir_cluster():
    ...
    if not ids:
        print("No hay instancias Kafka para terminar.")
        return

    print(f"Terminando {len(ids)} instancia(s) Kafka: {' '.join(ids)}")
    ec2_resource.instances.filter(InstanceIds=ids).terminate()


def main():
    parser = argparse.ArgumentParser(description="Gestor de infraestructura Kafka en EC2")
    grupo = parser.add_mutually_exclusive_group(required=True)
    grupo.add_argument("--start_nodes", type=int, metavar="N", help="Crea 1 master Kafka y N workers cliente")
    grupo.add_argument("--check", action="store_true", help="Lista las instancias Kafka creadas")
    grupo.add_argument("--delete", action="store_true", help="Termina las instancias Kafka creadas")
